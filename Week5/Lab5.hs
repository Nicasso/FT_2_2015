...
testMinimal = do [r] <- rsolveNs [emptyN]
                 s <- genProblem r
                 showNode s
                 if (uniqueSol s) == True then print ("P has a unique solution.") else error ("P does not have a unique solution, thus P is minimal")
                 if (tryRemovingHint s (filledPositions (fst s)) 0) == True then print ("All P' of P have multiple solutions, thus P is minimal") else error ("A P' of P also has a unique solution, thus P is not minimal")
